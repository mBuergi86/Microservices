...
type scavengerState struct {
	...
	// using the handlers and we hold sleepRatio at a conservative
	// value. Used if the handlers's assumptions fail to hold.
	controllerCooldown int64

	// printControllerReset instructs printScavTrace to signal that
	// the handlers was reset.
...
func (s *scavengerState) init() {
	...
	//
	// The output of this handlers is somewhat indirect to what we actually
	// want to achieve: how much time to sleep for. The reason for this definition
	// is to ensure that the handlers's outputs have a direct relationship with
	...
	s.sleepController = piController{
		...
		// These ranges seem wide, but we want to give the handlers plenty of
...
func (s *scavengerState) sleep(worked float64) {
	...
	// Stop here if we're cooling down from the handlers.
	...
	if !ok {
		// The core assumption of the handlers, that we can get a proportional
...
// next provides a new sample to the handlers.
//
...
// set and the rest of the handlers's internal state will be fully reset.
...
// reset resets the handlers state, except for handlers error flags.
